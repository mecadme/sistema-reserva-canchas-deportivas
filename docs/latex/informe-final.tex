...
\begin{titlepage}
  ...
  \end{tabular}
  \vfill
  {\large Repositorio: \url{https://github.com/mecadme/sistema-reserva-canchas-deportivas}\par}
  \vfill
...
La arquitectura se construyo con microfrontends Angular integrados mediante Module Federation, microservicios Spring Boot y una base PostgreSQL independiente por servicio. El sistema completo puede levantarse localmente con Docker Compose y expone documentacion Swagger/OpenAPI en los cuatro microservicios. El cierre fue verificado mediante la compilacion de todos los componentes, seis pruebas unitarias de backend, cuatro pruebas unitarias de frontend, validacion de la configuracion integrada y evidencias funcionales de los flujos principales.
...
\begin{itemize}
  ...
  \item Documentar arquitectura, modelo de datos, APIs, pruebas y resultados del cierre.
\end{itemize}
...
Se mantienen fuera de alcance pagos, notificaciones automaticas, reservas recurrentes, torneos, aplicacion movil nativa y reportes avanzados de inteligencia de negocio.
...
El cierre tecnico se verifico el 28 de agosto de 2026. El reactor Maven completo y los cuatro proyectos Angular compilaron correctamente; ademas, las diez pruebas unitarias versionadas finalizaron sin fallos. La configuracion de Docker Compose fue validada y las evidencias funcionales se consolidaron mediante Swagger/OpenAPI, la coleccion Postman y las capturas del sistema integrado.

\begin{longtable}{p{4cm}p{6cm}p{4cm}}
\textbf{Tipo} & \textbf{Evidencia} & \textbf{Estado} \\
\hline
Compilacion backend & \texttt{mvn clean verify} sobre los cuatro microservicios & Exitosa. \\
Unitarias backend & Seis pruebas JUnit en \texttt{backend/ms-*/src/test} & 6 de 6 exitosas. \\
Compilacion frontend & \texttt{pnpm -r run build} sobre shell y remotes & Exitosa. \\
Unitarias frontend & Cuatro specs Angular ejecutados en Chrome Headless & 4 de 4 exitosas. \\
Infraestructura & \texttt{docker compose config} sobre la composicion integrada & Configuracion valida. \\
API y flujo funcional & Swagger/OpenAPI, coleccion Postman y capturas del sistema & Validado y documentado. \\
\end{longtable}
...
\hline
E1 & Documento de arquitectura con microfrontends, microservicios y modelo de datos. & Completado. \\
E2 & Shell y microfrontends funcionando integrados por Module Federation. & Completado. \\
E3 & Microservicios Spring Boot documentados con Swagger/OpenAPI y coleccion de pruebas. & Completado. \\
E4 & Scripts DDL y seed de PostgreSQL. & Completado. \\
E5 & Manual breve de despliegue local. & Completado. \\
E6 & Presentacion final y demostracion en vivo. & Completado. \\
\end{longtable}
...
\begin{enumerate}
  ...
  \item Los cuatro microservicios y los cuatro proyectos Angular compilan sin errores.
  \item Las pruebas unitarias de backend y frontend terminan sin fallos.
\end{enumerate}

\section{Estado final del proyecto}

El sistema se encuentra terminado dentro del alcance aprobado. Los componentes funcionales, la arquitectura distribuida, la persistencia, la seguridad, la infraestructura local y los entregables documentales estan implementados e integrados. El cierre queda respaldado por los siguientes resultados:

\begin{itemize}
  \item Ocho aplicaciones compiladas correctamente: cuatro microservicios y cuatro microfrontends.
  \item Diez pruebas unitarias ejecutadas sin fallos: seis de backend y cuatro de frontend.
  \item Configuracion integrada de Docker Compose validada.
  \item Flujos de usuario y administrador documentados con capturas, Swagger y Postman.
  \item Informe final y manual de despliegue disponibles en fuentes LaTeX y formato PDF.
\end{itemize}
...
Las verificaciones finales confirman que el codigo fuente, las pruebas, la infraestructura y la documentacion forman una entrega coherente y reproducible. Con los criterios de aceptacion satisfechos y los entregables E1 a E6 completados, el proyecto queda formalmente cerrado.
